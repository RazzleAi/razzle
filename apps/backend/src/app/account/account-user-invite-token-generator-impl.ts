...
@Injectable()
export class AccountUserInviteTokenGeneratorImpl implements AccountUserInviteTokenGenerator {
    ...
    generateInviteToken(accountOwner: AccountUser, emailInvitee: string): AccountUserInviteToken {
        ...
        console.log(`Token Expiry Value: ${process.env.ACCOUNT_INVITE_TOKEN_EXPIRY_MS}`);
        ...
        return {
            ...
            emailInvitee,
            expiresOn: new Date(new Date().getTime() + Number(process.env.ACCOUNT_INVITE_TOKEN_EXPIRY_MS)),
            valid: true,
