...
public class RazzleBeansPostProcessor implements BeanPostProcessor {
  ...
  private Logger log = Logger.getLogger(RazzleBeansPostProcessor.class.getName());


  private final RazzleActionHandlersContainer handlerContainer;


  public RazzleBeansPostProcessor(RazzleActionHandlersContainer handlerContainer) {
  ...
  public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
    if (hasActions(bean)) {
      log.info("Encountered Bean with actions: " + beanName);
  ...
  private void registerActions(Object bean) {
    ...
    for (Method method : methods) {
      if (method.isAnnotationPresent(Action.class)) {
        ...
        log.info("Registering action: %s - %s".formatted(name, description));
        handlerContainer.registerHandler(
          name,
          new ActionHandler.Builder()
            .name(name)
